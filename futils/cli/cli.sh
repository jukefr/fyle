...
echo "fconvert/image/"
shift 2
docker run --volumes-from "$(hostname)" "fconvert/image:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "sound" ]; then
echo "fconvert/sound/"
shift 2
docker run --volumes-from "$(hostname)" "fconvert/sound:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "video" ]; then
echo "fconvert/video/"
shift 2
docker run --volumes-from "$(hostname)" "fconvert/video:v0.1-alpha9" "$@"
exit 0
fi
fi
...
echo "foptimize/gif/"
shift 2
docker run --volumes-from "$(hostname)" "foptimize/gif:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "jpg" ]; then
echo "foptimize/jpg/"
shift 2
docker run --volumes-from "$(hostname)" "foptimize/jpg:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "png" ]; then
echo "foptimize/png/"
shift 2
docker run --volumes-from "$(hostname)" "foptimize/png:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "svg" ]; then
echo "foptimize/svg/"
shift 2
docker run --volumes-from "$(hostname)" "foptimize/svg:v0.1-alpha9" "$@"
exit 0
fi
fi
...
echo "futils/alpha/"
shift 2
docker run --volumes-from "$(hostname)" "futils/alpha:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "alpine" ]; then
echo "futils/alpine/"
shift 2
docker run --volumes-from "$(hostname)" "futils/alpine:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "cli" ]; then
echo "futils/cli/"
shift 2
docker run --volumes-from "$(hostname)" "futils/cli:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "monochrome" ]; then
echo "futils/monochrome/"
shift 2
docker run --volumes-from "$(hostname)" "futils/monochrome:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "resize" ]; then
echo "futils/resize/"
shift 2
docker run --volumes-from "$(hostname)" "futils/resize:v0.1-alpha9" "$@"
exit 0
fi
if [ $2 = "rotate" ]; then
echo "futils/rotate/"
shift 2
docker run --volumes-from "$(hostname)" "futils/rotate:v0.1-alpha9" "$@"
exit 0
fi
fi
